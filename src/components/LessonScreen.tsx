...
export function LessonScreen({
  ...
  const stepLabels: Record<number, { tag: string; icon: string }> = {
    0: { tag: "MISSÃO", icon: "flag" },
    1: { tag: "QUIZ", icon: "quiz" },
    2: { tag: "SIMULAÇÃO", icon: "psychology" },
    3: { tag: "PRÁTICA", icon: "directions_car" },
  };

  const currentStepInfo = stepLabels[lessonStep] || stepLabels[0];
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="bg-card border-b border-border px-4 py-3 flex items-center justify-between">
        ...
        <button onClick={onBack} className="material-symbols-outlined text-muted-foreground hover:text-foreground transition-colors">
          close
        </button>
      </header>
      ...
      <div className="flex-1 flex flex-col lg:flex-row max-w-7xl mx-auto w-full">
        {/* LEFT SIDEBAR */}
        <aside className="lg:w-80 bg-card border-b lg:border-b-0 lg:border-r border-border p-5 flex flex-col gap-5">
          {/* Module info */}
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">Módulo Atual</p>
            <h2 className="text-lg font-bold text-foreground">{phase.title.replace(/Fase \d+ — /, "")}</h2>
          </div>

          {/* Confidence */}
          <div className="bg-accent/50 rounded-xl p-4 border border-border">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-bold text-muted-foreground">Confiança</span>
              <span className="text-xs font-extrabold text-primary">{Math.round(lessonProgress)}%</span>
            </div>
          ...
          {/* Steps progress */}
          <div>
            <p className="text-xs font-bold text-muted-foreground mb-3">Nível de Progresso</p>
              ...
              {STEPS.map((s, i) => (
                <div key={s.key} className="flex items-start gap-3">
                  <div className="flex flex-col items-center">
                    ...
                    {i < STEPS.length - 1 && <div className="w-0.5 min-h-[20px] bg-border my-0.5" />}
                  </div>
                  <div className="pb-3">
                    <p className={`text-sm font-semibold ${i <= lessonStep ? "text-foreground" : "text-muted-foreground"}`}>{s.label}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <main className="flex-1 p-5 md:p-8 overflow-auto">
          ...
          <div className="flex items-center gap-3 mb-6">
            <span className="inline-flex items-center gap-1.5 bg-primary/10 text-primary rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wider">
              <span className="material-symbols-outlined text-sm">{currentStepInfo.icon}</span>
              {currentStepInfo.tag}
            </span>
            ...
            {lessonStep === 1 && (
              <div>
                ...
                <div className="bg-card rounded-xl border border-border overflow-hidden mb-6">
                  <div className="bg-gradient-to-br from-[hsl(var(--blue-800))] to-[hsl(var(--blue-900))] aspect-video flex items-center justify-center relative">
                    ...
                    {/* Phase badge */}
                    <div className="absolute top-3 left-3 bg-primary text-primary-foreground text-[10px] font-extrabold uppercase tracking-widest px-2.5 py-1 rounded-md">
                      Fase {currentPhase + 1}
                    </div>
                  </div>
                ...
                <div className="bg-card rounded-xl border border-border p-5 md:p-6">
                  {isRetry ? (
                    <span className="inline-flex items-center gap-1.5 bg-accent text-accent-foreground rounded-full px-3 py-1 text-xs font-bold uppercase mb-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-6">
                  <div className="bg-card rounded-xl border border-border p-4 flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">Memorização e associação mecânica dos comandos.</p>
                    </div>
                  </div>
                  <div className="bg-card rounded-xl border border-border p-4 flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">Funcionamento básico do sistema de transmissão.</p>
                    </div>
                  </div>
                  <div className="bg-card rounded-xl border border-border p-4 flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">Aguardando sua interação para continuar.</p>
                    </div>
                  </div>
                </div>
              </div>
